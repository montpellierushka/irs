...
export default function Services ({ fetchInitialData, serverData }) {	
	...
	return __isBrowser__
		? ((window.__INITIAL_PATH__ === pathname) ? window.__INITIAL_DATA__.pageBackendContent : (stateData.servicesContext[pathname] === undefined) ? false : stateData.servicesContext[pathname])
		: serverData.pageBackendContent
	})
	
	if((stateData.servicesContext[pathname] === undefined) && __isBrowser__){
		if(window.__INITIAL_PATH__ === pathname){	
			stateData.servicesContext[pathname] = window.__INITIAL_DATA__.pageBackendContent; 
			stateData.contextSEO[pathname] = window.__INITIAL_DATA__.backendSEO;
			dispatchData({
				type: "FETCH_SERVICES",
				payload: stateData.servicesContext
			}) 
		}
	}
	...
	React.useEffect(() => {
		if (fetchNewContent.current === true) {
			if(!(stateData.servicesContext[pathname] === undefined)){
				setContent(stateData.servicesContext[pathname] )				 
			} else {
				setLoading(true)
				...
				.then((response) => {
					...
					setLoading(false)	
					stateData.servicesContext[pathname] = response.pageBackendContent;
				...
				<div className={classes.first__content}>
					...
					<div className={classes.columns}>
						<div className={classes.columns__column}>
							...
							<Link to="/services/sozdanie-prodayushhih-internet-magazinov" className={classes.columns__link}>Сайты на платформе Tilda Publishing.</Link>
