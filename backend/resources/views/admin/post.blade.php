...
<div class="row">
	<div class="col-md-12">
		<div class="card card-primary">
			...
			<form method="POST" class="new-theme-form" action="{{ route('admin_posts_new') }}">
                @csrf 
				<div class="card-body"> 
					<div class="form-group">
						...
						<select class="form-control select2" name="category_id" required>
							<?foreach($categories as $cat){?>
								<option value="<?=$cat->id?>" <?if(old('category_id') == $cat->id){ echo "selected"; }?>><?=$cat->title?></option>
							<?}?> 
						</select>
					</div>     
					<div class="form-group">
					...
					<div class="container-fluid">
						<div class="row">
							<div class="form-group col-8">
								...
								<input type="text" class="form-control @error('preview') is-invalid @enderror img-preview-input" name="preview" value="{{ old('preview') }}" placeholder="Ссылка на картинку">
							</div>
					...
					<div class="form-group">
						<label>Дата поста @error('date') <code>{{ $message }}</code> @enderror</label>
							<div class="input-group date" id="reservationdatetime" data-target-input="nearest">
								<input type="text" class="form-control datetimepicker-input @error('date') is-invalid @enderror" data-target="#reservationdatetime"  name="date" value="{{ old('date') }}" required />
					...
					<div class="form-group">
						...
						<textarea class="form-control ta-theory" rows="3" placeholder="Теория" name="content">{{ old('content') }}</textarea>
					</div> 
				</div> 
