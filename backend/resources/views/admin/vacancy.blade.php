...
<div class="row">
	<div class="col-md-12">
		<div class="card card-primary">
			...
			<form method="POST" class="new-theme-form" action="{{ route('admin_vacancies_update') }}">
                @csrf 
				<div class="card-body">    
					<input type="hidden" value="{{ $vacancy->id }}" name="id">
					<div class="form-group">
						<label>Название @error('title') <code>{{ $message }}</code> @enderror</label>
						<input type="text" class="form-control mytranslit @error('title') is-invalid @enderror" name="title" value="{{ $vacancy->title }}" placeholder="" required>
					</div>  
					<div class="form-group">
						<label>Символьный код @error('slug') <code>{{ $message }}</code> @enderror</label>
						<input type="text" class="form-control mytranslitto @error('slug') is-invalid @enderror" name="slug" value="{{ $vacancy->slug }}" placeholder="" required>
					</div>     
					<div class="form-group">
						<label>Краткое описание @error('excerpt') <code>{{ $message }}</code> @enderror</label>
						<input type="text" class="form-control @error('excerpt') is-invalid @enderror" name="excerpt" value="{{ $vacancy->excerpt }}" placeholder="" required>
					</div>  
					<div class="form-group">
						<label>SEO Тайтл (если пуст выводится тайтл по умолчанию) @error('seo_title') <code>{{ $message }}</code> @enderror</label>
						<input type="text" class="form-control @error('seo_title') is-invalid @enderror" name="seo_title" value="{{ $vacancy->seo_title }}" placeholder="">
					</div>  
					<div class="form-group">
						...
						<input type="text" class="form-control @error('seo_description') is-invalid @enderror" name="seo_description" value="{{ $vacancy->seo_description }}" placeholder="">
					</div>    	
					<div class="form-group">
						<label>Контент @error('content') <code>{{ $message }}</code> @enderror</label>
						<textarea class="form-control ta-theory" rows="3" placeholder="Контент" name="content">{{ $vacancy->content }}</textarea>
					</div> 
				</div> 
